...
このシステムでは、音声の文字起こしに Whisper を使用します。

### Whisper のモデルサイズ

Whisper のモデルサイズは `MODEL_SIZE` で設定できます。設定しない場合は `small` が使用されます。
モデルが大きいほど一般的に精度は向上しますが、初回ダウンロードに必要な時間、メモリ使用量、処理時間も増加します。

利用できる主なモデルサイズは次のとおりです。
...
| `base`     | 軽量と精度のバランス                   |
| `small`    | より高い精度。既定値                   |
| `medium`   | 高精度。より多くのメモリが必要         |
| `large-v3` | 最高精度。大きなメモリと処理時間が必要 |

ローカル開発では `services/controller/.env` に設定します。

```env
MODEL_SIZE=base
```

Docker Compose では、プロジェクトルートの `.env` に設定します。
